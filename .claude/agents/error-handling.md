...
Ce dépôt **est** un plugin Claude Code : cinq scripts `sh` dans `hooks/`, du
Markdown, deux manifestes JSON. La convention de gestion d'erreurs y est
inhabituelle et prime sur tes réflexes habituels.

- **Un hook ne doit jamais faire échouer une session.** Toute condition
  inattendue — stdin vide, JSON tronqué, `CLAUDE_PROJECT_DIR` absent,
  répertoire en lecture seule, `jq` manquant — se traduit par une **sortie
  silencieuse en code 0**, jamais par un message d'erreur ni un code non nul.
  Le seul code non nul volontaire serait un blocage `Stop`, et il passe par du
  JSON sur stdout, pas par un code de sortie.
- **Aucune dépendance obligatoire.** `jq` est utilisé s'il est présent, avec
  repli `sed`/`awk` (`_HAS_JQ` dans `lib.sh`). Tout ajout d'outil externe doit
  suivre le même schéma, ou être refusé.
- **`set -u` est en place, `set -e` volontairement absent** : un `-e` ferait
  échouer un hook sur une commande bénigne. Les échecs se gèrent explicitement
  avec `|| true` ou `|| exit 0`.
- **Les timeouts sont dans `hooks/hooks.json`** (10 s). Un traitement qui pourrait
  les dépasser doit être borné dans le script, pas espéré court.
- `sh -n` ne prouve rien sur la robustesse : teste par invocation réelle avec un
  payload malformé sur stdin.
